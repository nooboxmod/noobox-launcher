...
Icon.prototype = {
  ...
  appendNotifications: function icon_appendNotifications(num) {
    var notificationsNum = num;
    if (notificationsNum > 99) {
      notificationsNum = 99;
    }

    var notifications = this.container.querySelector('.notifications');
    if (notifications && num) {
      // Update number for notifications.
      notifications.textContent = notificationsNum;
      return;
    }
    ...
    notifications.dataset.isNotifications = true;
    notifications.textContent = notificationsNum;
    this.container.appendChild(notifications);
  },

  removeNotifications: function icon_appendNotifications() {
    var notifications = this.container.querySelector('.notifications');
    if (!notifications) {
      return;
    }

    this.container.removeChild(notifications);
  },
  ...
  renderImageForBookMark: function icon_renderImageForBookmark(img) {
    ...
    background.onload = function icon_loadBackgroundSuccess() {
      ...
      //tcl_WCL modified for pr587069 begin
      ctx.mozImageSmoothingEnabled = !(img.naturalWidth === 32 * SCALE_RATIO &&
          img.naturalHeight === 32 * SCALE_RATIO);
      //tcl_WCL modified for pr587069 end
