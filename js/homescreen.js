...
var Homescreen = (function() {
  ...
  function initialize(lPage, onInit) {
    ...
    GridManager.init(options, function gm_init() {
      ...
      // communication with system app to get notifications for sms and dialer.
      navigator.mozApps.getSelf().onsuccess = function(evt) {
        var app = evt.target.result;
        app.connect('homescreen-notifications-comms').then(
          function onConnAccepted(ports) {
            ports.forEach(function(port) {
              port.postMessage('get-notifications-num');
            });
          }, function onConnRejected(reason) {
            console.log('homescreen is rejected');
            console.log(reason);
          });
      };

      // communication with system app to get notifications for sms and dialer.
      window.addEventListener('iac-notificationscomms', function(evt) {
        var message = evt.detail;
        var manifestURL = message.manifestURL;
        var num = message.num;

        // For sms notifications.
        if (manifestURL === 'app://sms.gaiamobile.org/manifest.webapp') {
          var descriptor = {manifestURL: manifestURL};
          // If there's an existing icon for this app already,
          // let it update itself.
          var existingIcon = GridManager.getIcon(descriptor);
          if (existingIcon) {
            if (num === 0) {
              existingIcon.removeNotifications();
            } else {
              existingIcon.appendNotifications(num);
            }
          }
        }

        // For dialer notifications.
        if (manifestURL ===
          'app://communications.gaiamobile.org/manifest.webapp') {
          var descriptor = {manifestURL: manifestURL, entry_point: 'dialer'};
          // If there's an existing icon for this app/entry point already,
          // let it update itself.
          var existingIcon = GridManager.getIcon(descriptor);
          if (existingIcon) {
            if (num === 0) {
  ...
  // added by tcl_chenguoqiang 780824
  function forceOrientation() {
    if (document.hidden == false) {
      dump('cgq forceOrientation');
      window.screen.mozLockOrientation('default');
    }
  }

  window.addEventListener('resize', function forceResize() {
     setTimeout(forceOrientation, 200);
  });
  // end added
