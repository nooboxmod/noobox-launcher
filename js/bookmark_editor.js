...
var BookmarkEditor = {
  ...
  _init: function bookmarkEditor_init() {
    ...
    // tcl_longxiuping add for bug 712043 begin
    if (this.bookmarkTitle.value.trim() === '') {
      this.addButton.setAttribute('disabled', 'disabled');
    }

    this.bookmarkTitle.addEventListener('input',
      this.addRemoveDisabled.bind(this));
  },

  addRemoveDisabled: function bookmarkEditor_addRemoveDisabled() {
    if (this.bookmarkTitle.value.trim() === '') {
      this.addButton.setAttribute('disabled', 'disabled');
    } else {
      this.addButton.removeAttribute('disabled');
  ...
  // tcl_longxiuping add for bug 712043 end
  ...
  save: function bookmarkEditor_save(evt) {
    // tcl_longxiuping delete for bug 712043
    // because if the url is not valid, user can not add it again.
