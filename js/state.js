...
const HomeState = (function() {
  ...
  // tcl_longxiuping add for bug 757515 begin.
  function updateDB(iterator, success, error) {
    var grid = Configurator.getSection('grid') || [];

    // add the collection page(FFOS1.3 new add) from the configurator
    for (var i = 0; i < grid.length; i++) {
      grid[i] = {
        index: i,
        icons: grid[i]
  ...
  // tcl_longxiuping add for bug 757515 end.
  ...
  function openDB(success, error) {
    ...
    var request;
    var emptyDB = false;
    var upgradeDB = false; // tcl_longxiuping add for bug 757515.
      ...
      switch (oldVersion) {
        ...
        case 1:
          ...
          if (!db.objectStoreNames.contains(SV_APP_STORE_NAME)) {
            ...
            upgradeDB = true; // tcl_longxiuping add for bug 757515.
          }
      }
    };
  }
  ...
  return {
    ...
    init: function st_init(iteratorGrid, success, error, iteratorSVApps) {
      // tcl_longxiuping modified for bug 757515 begin.
      openDB(function(emptyDB, upgradeDB) {
        dump('lxp:: emptyDB ' + emptyDB + '  upgradeDB ' + upgradeDB);
        if (emptyDB) {
        ...
        // tcl_longxiuping modified for bug 757515 end.
